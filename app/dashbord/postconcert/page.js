...
export default function PostConcert() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const authRes = await fetch('/api/sign-image', { method: 'POST' });
      ...
      cldFormData.append('folder', 'concert_posters');
      ...
      if (res.ok) {
        alert("Concert enregistré avec succès !");
        // Reset complet
        setFormData({ title: '', date: '', time: '', location: '', description: '', entryType: 'Libre' });
  ...
  return (
    <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
      <div className="card-header bg-primary text-white py-3">
        <h5 className="mb-0"><i className="bi bi-calendar-plus me-2"></i>Nouveau Concert</h5>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            ...
            <div className="col-12 mb-3">
              <label className="form-label fw-bold small text-primary">Affiche du Concert</label>
              <input 
                type="file" 
                ref={fileInputRef}
                accept="image/*" 
            ...
            <div className="col-12">
              <label className="form-label fw-bold small">Nom de l'événement</label>
              <input 
                type="text" name="title" className="form-control shadow-sm" 
                placeholder="Ex: Grand Concert de Noël" required
            ...
            <div className="col-md-6">
              <label className="form-label fw-bold small">Heure</label>
              <input 
                type="time" name="time" className="form-control shadow-sm" required
            ...
            {/* Type d'entrée */}
            <div className="col-12">
              <label className="form-label fw-bold small">Type d'entrée</label>
              <select 
                className="form-select shadow-sm" name="entryType" 
                value={formData.entryType} onChange={handleChange}
                disabled={loading}
              >
                <option value="Libre">Entrée Libre</option>
                <option value="Payant">Entrée Payante</option>
              </select>
            </div>
            ...
            <div className="col-12 mt-4">
              ...
              >
                {loading ? (
                  ...
                  <><i className="bi bi-check-circle me-2"></i>Enregistrer le concert</>
                )}
              </button>
            </div>
          </div>
